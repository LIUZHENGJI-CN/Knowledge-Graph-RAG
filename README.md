...
- 论文内容类问题：通过向量检索召回相关论文，再由 LLM 基于论文摘要、背景、方法和关键词生成回答。
- 图谱结构类问题：通过 Text2Cypher 查询作者、机构、分类、关键词、发表渠道和合作关系等结构化信息。

当前限制：

- 前端展示为聊天形式，但后端暂不支持真正的多轮上下文记忆。
- Fast RAG 当前没有直接使用 `Paper.full_text`，主要使用摘要和语义抽取字段。
- 当前系统通过关键词判断问题应进入知识图谱查询、语义问答或混合问答流程，因此对表达含糊或同时包含多种意图的问题，可能出现路由不完全准确的情况。
- LLM 抽取的 `background`、`methods`、`keywords` 可能存在噪声，结果需要结合原论文核验。
- Text2Cypher 生成的 Cypher 已做只读限制，但仍可能因问题表达复杂或 schema 覆盖不足而生成不理想查询。
